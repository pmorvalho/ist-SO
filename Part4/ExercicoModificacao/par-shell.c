/*
SHELL PARALELA - EXERCICIO 4 MODIFICADO 
Grupo 40:
81151 - Pedro Orvalho
81365 - Ana Leitao
...
FILE *file;
int iteration = -1, total_exec_time = 0, succ_children = 0;
...
void *monitor_cicle() { 
  ...
  while (1) {
    ...
    if (exit_flag && numchildren == 0) { /* verifica se a flag do exit ja se encontra activa e se nao ha filhos ativos*/
      ...
      if(status == 0)
      	succ_children++;
      fprintf(file, "iteracao %d\npid: %d execution time: %.0f s\ntotal execution time: %d s\nsuccessful children: %d\n", ++iteration, childpid, exec_time, total_exec_time, succ_children); /*faz a escrita no ficheiro sobre a nova iteracao e o tempo total de execucao*/
      fflush(file);
...
int main (int argc, char** argv) {
  ...
  while( fgets(line, LINE_MAX_SIZE, file) != NULL ) {  /* ciclo que vai buscar a ultima iteracao e o ultimo tempo total de execucao
    ...
    sscanf(line, "successful children: %d", &succ_children); 
  }
