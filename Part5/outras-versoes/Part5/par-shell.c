...
int main (int argc, char** argv) {
  ...
  while (1) {
    ...
    if ((n = read(fileServ, buffer, BUFFER_SIZE)) < 0)
    ...
    for(i = 0; i<BUFFER_SIZE; i++)  /* retira a letra de controlo do comando*/
      buffer[i] = buffer[i+1]; 
    ...
    if (strcmp(args[0], EXIT_COMMAND_GLOBAL) == 0) { /*verifica se foi introduzido o comando 'exit'*/
      exitGlobal();
    }
